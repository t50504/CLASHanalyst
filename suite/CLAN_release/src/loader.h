...
class Loader  {
 public:
  Loader();
  ~Loader();
  bool IsFASTA(const char *file_name);
  bool IsFASTQ(const char *file_name);
  // return the number of sequences in the FASTA file
  // file_name: the name of the file to be read
  int CountFastaNumSeqs(const char *file_name);
  int CountFastqNumSeqs(const char *file_name);
  ...
  int LoadFasta(BioAlphabet &alphabet, const char *file_name, char **header, char **seq, const int min_len = 10);
  int LoadFastq(BioAlphabet &alphabet, const char *file_name, char **header, char **seq, const int min_len = 10);
